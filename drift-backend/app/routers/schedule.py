...
def get_overall_schedule(
    ...
    """
    blocks = scheduler.generate_rescue_schedule(db, current_user.id)
    return blocks
...
def get_task_schedule(
    ...
    suggestion = db.query(models.ScheduleSuggestion).filter(
        models.ScheduleSuggestion.task_id == task_id
    ).first()
    
    if not suggestion:
        # If no schedule exists yet, run it on-the-fly
        blocks = scheduler.generate_rescue_schedule(db, current_user.id)
...
def regenerate_schedule(
    ...
    for task in active_tasks:
        ...
        db.query(models.ScheduleSuggestion).filter(
            models.ScheduleSuggestion.task_id == task.id
        ).delete()
